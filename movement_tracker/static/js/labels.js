...
const manoViewer = (() => {
    ...
    function _updateJerkTooltip(e) {
        ...
        let txt = `f${hit.gf}`;
        if (m.power != null) txt += ` · deviates ${Math.round(m.power * 100)}% of movement`;
    ...
    async function _recombineMpCurrentTrial() {
        ...
        // Refresh the active trial in place so the newly-written
        // mediapipe_combined.npz is picked up without a page reload.
        if (ok) {
