"""
Import existing DLC labels into the hand-tracker SQLite database.
...
Usage:
    python -m movement_tracker.import_labels              # all projects
    python -m movement_tracker.import_labels MSA18 Con01   # specific projects
    python -m movement_tracker.import_labels --dry-run     # preview without writing
...
def import_project(db, subject_name: str, dlc_path: Path, dry_run: bool = False) -> dict:
    ...
    if not subj:
        stage = infer_stage(dlc_path, subject_name=subject_name)
        if not dry_run:
            db.execute(
