...
def save_events(session_id: int, body: dict = Body(...)) -> dict:
    ...
    # "events_complete" requires all 3 event types to have data in EVERY trial.
    ...
    if types_with_data:
        if len(types_with_data) < len(AUTO_DETECT_TYPES):
            ...
            trials = build_trial_map(subject_name)
            all_trials_covered = True
            for trial in trials:
                s, e = trial["start_frame"], trial["end_frame"]
                for et in AUTO_DETECT_TYPES:
                    if not any(s <= f <= e for f in existing.get(et, [])):
                        all_trials_covered = False
                        break
                if not all_trials_covered:
                    break
            new_stage = "events_complete" if all_trials_covered else "events_partial"
        with get_db_ctx() as db:
            session = db.execute(
...
def detect_events(session_id: int, body: dict = Body(default={})) -> dict:
    ...
    # Valley-aware: split when a deep valley separates nearby candidates
...
def detect_events_v2(session_id: int, body: dict = Body(...)) -> dict:
    ...
    Request body: {
        "trial_index": int,
        "peaks_only": bool (default false),
        "enforce_sequence": bool (default true),
    ...
    for etype in merge_types:
        ...
        if trial_saved:
            # Type already has saved events for this trial — keep them as-is
            detected[etype] = trial_saved
        # else: use detected events (nothing saved to preserve)

    # Anchor-based close detection: when opens (or peaks) are saved but
    ...
    if not trial_saved_closes and not peaks_only and detected["peak"]:
        ...
        sorted_peaks = sorted(detected["peak"])
        sorted_opens = sorted(detected["open"])
        closes: list[int] = []

        for pk_g in sorted_peaks:
            # Search from peak to the next open (or trial end)
            next_opens = [o for o in sorted_opens if o > pk_g]
            end_g = next_opens[0] if next_opens else ef + 1
            lo = pk_g - sf
            hi = min(end_g - sf, len(d))
            if hi > lo + 1:
                close_local = min(range(lo, hi), key=lambda f: d[f])
                closes.append(sf + close_local)

        detected["close"] = sorted(set(closes))
    ...
    if not peaks_only and enforce_sequence:
        ...
        detected = valid_events

    return {
        "open": sorted(detected.get("open", [])),
        "peak": sorted(detected.get("peak", [])),
        "close": sorted(detected.get("close", [])),
    }
